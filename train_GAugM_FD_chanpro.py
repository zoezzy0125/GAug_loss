...
def constractive_search(generated_indexes, features, adj_orig):
    ...
    for i in range(len(U_generate_list)):
        euclidean_distance_list.append(euclidean_distance(LED_ori,LED_generate_list[i]))
    generated_adj = generated_indexes[euclidean_distance_list.index(min(euclidean_distance_list))]
...
if __name__ == "__main__":
    ...
    for _ in range(5):
        ...
        gnn = GNN(adj_orig, adj_orig, features, labels, tvt_nids, print_progress=True, cuda=int(args.gpu), epochs=200, ways='changepro',adj_orig=adj_orig, params=params, A_pred=A_pred)
        acc, _, _ = gnn.fit()
        accs.append(acc)
