    ...
    def arguments_read(self, *args, **kwargs):
        ...
        if not data:
            ...
            if not args:
                ...
                x, edge_index, batch, perturb = args[0],args[1],args[2],args[3]
                #perturb=None
            else:
        ...
        return x, edge_index, batch, perturb

class GCN_3l_flag(GNNBasic):

    def __init__(self, model_level, dim_node, dim_hidden, num_classes,dropping_rate=0): 
        #dim_node = in_channels; dim_hidden = hidden_channels out_channels = num_classes
        super().__init__()
        num_layer = 3
        ...
        self.ffn = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Dropout(dropping_rate)
            #nn.Linear(dim_hidden, num_classes)
        )
        self.final_li=nn.Linear(dim_hidden,num_classes)
        self.dropout = nn.Dropout()
        self.embed = torch.nn.Linear(dim_node,dim_node)
    ...
    def forward(self, *args, **kwargs) -> torch.Tensor:
        ...
        x, edge_index, batch, perturb = self.arguments_read(*args, **kwargs)
    ...
    def get_emb(self, *args, **kwargs) -> torch.Tensor:
        ...
        post_conv = self.relu1(self.conv1(x, edge_index))
        for conv, relu in zip(self.convs, self.relus):
            post_conv = relu(conv(post_conv.clone(), edge_index))
        return post_conv


class GCN_3l_kmeans(GNNBasic):
...
class GCN_3l(GNNBasic):
    ...
    def forward(self, *args, **kwargs) -> torch.Tensor:
        ...
        out_readout = self.readout(post_conv, batch)
        out_embed = self.ffn(out_readout)
        out = self.final_li(out_embed.clone())
        return out,out_embed
...
class GCNConv(gnn.GCNConv):
    ...
    def forward(self, x: Tensor, edge_index: Adj,
        ...
        x = torch.matmul(x.clone(), self.weight.clone())
