...
from models.GCN_dgl import GCNLayer
...
class PGExplainer_DropEdge(nn.Module):
    r"""
    ...
    def __init__(self, model, in_channels: int, device, explain_graph: bool = True, epochs: int = 20,
        ...
        super(PGExplainer_DropEdge, self).__init__() 
        self.model = model
        self.device = device
    ...
    def __set_masks__(self, x: Tensor, edge_index: Tensor, edge_mask: Tensor = None):
        ...
        modules1, modules2 = tee(self.model.modules,2)
        for module in modules1:
            if isinstance(module, GCNLayer):
                module._explain = True
                module.__edge_mask__ = self.edge_mask
        self.model.modules=modules2
    ...
    def __clear_masks__(self):
        ...
        self.edge_mask = None
        self.model.modules=modules2
        print("____________clear_______masks__________")
    ...
    def get_subgraph(self,
        ...
        #print("num nodes {} num edges {}".format(num_nodes,edge_index.shape))
        graph = to_networkx(data=Data(x=x, edge_index=edge_index), to_undirected=True)
        #print("edge index before k-hop",edge_index,edge_index.shape)
        #print("self.hops",self.num_hops)
        subset, edge_index, _, edge_mask = k_hop_subgraph_with_default_whole_graph(
            edge_index, node_idx, self.num_hops, relabel_nodes=True,
            num_nodes=num_nodes, flow=self.__flow__())
        #print("edge index after k-hop",edge_index)
    ...
    def train_explanation_network(self, dataset):
        ...
        if self.explain_graph:
            ...
            with torch.no_grad():
                #data = dataset[0]
                data=dataset
                #print("train_explaination_network",self.model)
                data.to(self.device)
                self.model.eval()
            ...
            for epoch in range(self.epochs):
                ...
                for iter_idx, node_idx in tqdm.tqdm(enumerate(explain_node_index_list)):
                    with torch.no_grad():
                        #print("x before subgraph",data.x.shape) #2708,1433
                        #print("edge_index before subgraph",  data.edge_index.shape) #2,13264
                        x, edge_index, y, subset, _ = \
                            self.get_subgraph(node_idx=node_idx, x=data.x, edge_index=data.edge_index, y=data.y)
                        #print("x before shape",x.shape) #80,1433
                        #print("edge_index before shape",edge_index.shape) #2,298
