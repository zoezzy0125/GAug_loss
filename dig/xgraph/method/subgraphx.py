...
'''
如果未提供节点数量，则从 edge_index 中获取节点数量。

根据给定的流方向选择 row 和 col，这两个张量用于表示边的起始节点和目标节点。

创建布尔类型的节点掩码 node_mask 和边的掩码 edge_mask，用于标识子图中的节点和边。

如果未提供中心节点 node_idx，则通过遍历构建 k-跳子图：

首先创建一个初始的子图节点集合 subsets，初始化为只包含一个节点 0。
在一个循环中，不断扩展节点集合，通过将当前节点集合 subsets 中的节点标记为 True，并使用这些节点标记 row 张量，得到边的掩码 edge_mask。
将新的节点添加到 subsets 中，并将其保持唯一。如果新的节点集合 subsets 与之前的节点集合相等，表示已经构建完成，返回子图节点集合 subset。
如果提供了中心节点 node_idx，则根据中心节点构建 k-跳子图：

根据中心节点 node_idx 构建初始的子图节点集合 subsets。
在循环中，通过将当前节点集合 subsets 中的节点标记为 True，得到边的掩码 edge_mask。
将扩展后的节点添加到 subsets 中，并使用 torch.cat 连接这些节点，然后去除重复节点并返回节点集合 subset 和节点索引映射 inv。
更新节点掩码和边掩码，以及边连接 edge_index，保留在子图中的边。

如果需要重新编号节点，则为子图中的节点重新编号，然后更新边连接 edge_index。

返回节点集合 subset、边连接 edge_index、节点索引映射 inv 和边掩码 edge_mask。

总之，k_hop_subgraph_with_default_whole_graph 函数用于计算给定中心节点的 k-跳子图，并返回子图中的节点、边连接以及其他相关信息。
'''
...
class MCTS(object):
    ...
    '''
    1. 首先，获取当前树节点的节点集合 cur_graph_coalition，检查子图的节点数是否小于等于 min_atoms，如果是，直接返回该节点的奖励 P。

    2. 如果当前节点的子节点数为零，表示该节点尚未被访问，执行子节点的扩展过程。
        首先，获取子图中各节点的度，并按照度降序排序，生成节点列表 all_nodes。
        根据是否有特定的新节点索引，筛选需要扩展的节点。
        如果节点数超过 expand_atoms，则将扩展节点列表截断为前 expand_atoms 个节点。
        遍历每个扩展节点，对去除当前节点的子图进行剪枝和连通分量划分，保留最大的连通分量作为新的子图。
        检查是否已存在相同子图
    ...
    '''
    1. 如果 verbose 为 True，则在开始时打印图中节点的数量。

    2. 循环执行多次随机探索，总共进行 n_rollout 次。

    3. 在每次循环中，从根节点开始进行随机探索，调用 mcts_rollout 函数。这将模拟随机探索过程，扩展树节点并更新节点的统计信息。

    4. 如果 verbose 为 True，则在每次随机探索结束后打印当前探索次数和已经探索的不同子图状态数量。

    5.在所有的随机探索结束后，将所有已经探索过的子图节点按照它们的分数从高到低排序，并返回排序后的列表，这些子图节点表示对子图重要性的估计。

    6.总之，mcts 方法通过执行多次随机探索，获取对子图重要性的估计。每次探索都会扩展树节点，并在随机探索过程中更新节点的统计信息。最终，根据节点的分数进行排序，得到对子图重要性的估计列表。
    '''
...
class SubgraphX(object):
    ...
    '''
    这个函数 read_from_MCTSInfo_list 主要用于将序列化的 MCTS 
    节点信息重新读取为 MCTSNode 对象的列表。
    这个函数适用于接受保存的 MCTS 节点信息，
    将其反序列化并转换为 MCTSNode 对象，以便后续在程序中使用
    '''
