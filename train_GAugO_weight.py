...
if __name__ == "__main__":
    ...
    parser.add_argument('--fidF', type=float, default='1') #description='fidelity fraction'
    parser.add_argument('--kl_score', type=str, default='solo') #bi for gau-ori; solo for gau only
    parser.add_argument('--gau_mask', type=int, default=1)
    parser.add_argument("--linfball", type=float, default=1)
    parser.add_argument('--sample_ori_mask', type=int, default=1)
    ...
    if args.sample_type == 'diversity_enhanced' or args.sample_type == 'add_sample_fid' or args.sample_type == 'diversity_enhanced_sim':
    ...
    for big_epoch in range(total_big_epoch):
        model = GAug(adj_orig, features, labels, tvt_nids, cuda=gpu, epochs=args.epoch,
                     ...
                     temperature=params['temp'], warmup=0, gnnlayer_type=layer_type, 
                     jknet=jk, lr=lr, n_layers=n_layers, log=False, 
                     feat_norm=feat_norm, sample_type=args.sample_type, 
                     edge_weights=edge_weights, fid_div = fid_div, patience=args.patience)
