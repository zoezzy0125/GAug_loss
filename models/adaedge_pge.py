...
class AdaEdge(object):
    def __init__(self, adj_matrix, features, labels, tvt_nids, gnn, add_first, n_add, n_remove, conf_add, conf_remove, max_t=5):
        ...
        if self.gnn == 'gcn':
            ...
            self.GNN = JKNet
        self.model = self.GNN(self.adj, self.adj, self.features, self.labels, self.tvt_nids, print_progress=False, cuda=0)
        self.modules=self.model.modules
        self.data=self.model.data
       # print("initializing....",self.model)

    def fit(self):
        # self.model = GNN(self.adj, self.adj, self.features, self.labels, self.tvt_nids, print_progress=False, cuda=0)
        test_acc_init, val_acc_init, logits = self.model.fit()
        adj_new = self.adjustGraph(self.adj, logits)
        best_val_acc = val_acc_init
        test_acc = test_acc_init
        for i in range(1, self.max_t):
            self.model = self.GNN(adj_new, adj_new, self.features, self.labels, self.tvt_nids, print_progress=False, cuda=0)
            test_acc_i, val_acc_i, logits = self.model.fit()
    ...
    def get_emb(self,x,edge_index):
        print("indise adaEdge",edge_index.shape)
        return self.model.get_emb(x,edge_index)
