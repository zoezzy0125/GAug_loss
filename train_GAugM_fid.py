...
class AdjDecision(nn.Module):
    def __init__(self, input_size, hidden_size, output_size):
        super(AdjDecision, self).__init__()
        self.fc1 = nn.Linear(input_size, hidden_size)
        self.fc2 = nn.Linear(hidden_size, hidden_size)
        self.fc3 = nn.Linear(hidden_size, output_size)
    
    def forward(self, x):
        x = torch.triu(torch.tensor(x)).view(x.size(0),-1)
...
if __name__ == "__main__":
    ...
    A_pred_binary = (A_pred > 0.68819).astype(int)
    print(A_pred, A_pred_binary, np.sum(A_pred_binary==1))
