...
def sample_graph_det(adj_orig, A_pred, remove_pct, add_pct):
    ...
    if add_pct:
        ...
        #edges_pred = np.concatenate((edges_pred, np.array([edges_pred[1],edges_pred[0]])), axis=0)
...
if __name__ == "__main__":
    ...
    # mask, new_edges = sample_graph_det(adj_orig, A_pred, params['rm_pct'], params['add_pct'])
    # with open(args.dataset+"_mask.pkl", 'wb') as f:
    #     pickle.dump(np.array(mask),f)
    # print(np.array(new_edges).T)
    # with open(args.dataset+"_new_edges_from_GAugM.pkl", 'wb') as f:
    #     pickle.dump(np.array(new_edges).T, f)
    # print(torch.tensor(np.array(adj_pred.T)))
    # edges_pred = sample_graph_det(adj_orig, A_pred, params['rm_pct'], params['add_pct'])
    adj_pred = sample_graph_det(adj_orig, A_pred, params['rm_pct'], params['add_pct'])
    # edges_pred = np.array(edges_pred).T
    # edges_pred = np.concatenate((edges_pred, np.array([edges_pred[1],edges_pred[0]])), axis=1)
    # print(edges_pred.shape)
    #print(adj_pred.toarray(), np.array(adj_pred))
    with open(args.dataset+"_GAugM_adj.pkl", 'wb') as f:
        pickle.dump(adj_pred.toarray(), f)
    with open(args.dataset+"_sparse_GAugM_adj.pkl",'wb') as f:
        pickle.dump(adj_pred, f)
    # gnn = args.gnn
    # if gnn == 'gcn':
    #     GNN = GCN
    # elif gnn == 'gat':
